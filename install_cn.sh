...
main() {
    ...
    LATEST_URL="https://download.yunisdu.com/flash-cat/latest"
    
    # Get latest version
    VERSION=$(curl -s "$LATEST_URL")
    
    # Construct binary name
    BINARY_NAME="flash-cat-cli-${OS}-${VERSION}-${ARCH}.tar.gz"
    echo -e "${BLUE}Looking for asset: $BINARY_NAME${NC}"
    
    # Get download URL directly
    DOWNLOAD_URL="https://download.yunisdu.com/flash-cat/${VERSION}/${BINARY_NAME}"
