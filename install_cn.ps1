...
function Install-FlashCat {
    ...
    # Get latest release
    try {
        $latestRelease = Invoke-RestMethod -Uri "https://download.yunisdu.com/flash-cat/latest"
        Write-Host "Found latest release: $($latestRelease)" -ForegroundColor Cyan
        $version = $latestRelease.Trim()
        
        # Look for Windows binary with our architecture
        Write-Host "Version: $version" -ForegroundColor Cyan
        $binaryName = "flash-cat-cli-windows-${version}-${arch}.zip"

        $downloadUrl = "https://download.yunisdu.com/flash-cat/${version}/${binaryName}"
